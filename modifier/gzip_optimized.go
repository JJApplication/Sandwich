...
func NewOptimizedGzipModifier() *OptimizedGzipModifier {
	...
	log.InfoF("优化版Gzip修改器已初始化: enabled=%v, level=%d, threshold=%d, asyncThreshold=%d", 
		modifier.enabled, modifier.level, modifier.threshold, modifier.asyncThreshold)
...
func (g *OptimizedGzipModifier) UpdateConfig() {
	...
	if oldLevel != g.level {
		...
		log.DebugF("优化版gzip压缩级别已更新，writer对象池已重新初始化: level=%d", g.level)
	}
	
	log.DebugF("优化版gzip配置已更新: enabled=%v, level=%d, types=%v", g.enabled, g.level, g.types)
}
